...
def detectPlatesInScene(imgOriginalScene):
    ...
    if Main.showSteps1 == True: # show steps #######################################################
        cv2.imshow("1a-Escala Grises", imgGrayscaleScene)
        cv2.imshow("1b-filtro Umbral", imgThreshScene)
    # end if # show steps #########################################################################
    ...
    if Main.showSteps1 == True: # show steps #######################################################
        ...
        cv2.imshow("2b-AllConTour", imgContours)
    ...
    if Main.showSteps1 == True: # show steps #######################################################
        ...
        cv2.imshow("3- contornosChar", imgContours)
    # end if # show steps #########################################################################
    ...
    if Main.showSteps1 == True: # show steps #######################################################
        print "\n"
        cv2.imshow("4a-sectorPlaca", imgContours)
###Imprime todas las placas encontradas
        for i in range(0, len(listOfPossiblePlates)):
            p2fRectPoints = cv2.boxPoints(listOfPossiblePlates[i].rrLocationOfPlateInScene)

            cv2.line(imgContours, tuple(p2fRectPoints[0]), tuple(p2fRectPoints[1]), Main.SCALAR_RED, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[1]), tuple(p2fRectPoints[2]), Main.SCALAR_YELLOW, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[2]), tuple(p2fRectPoints[3]), Main.SCALAR_WHITE, 2)
            cv2.line(imgContours, tuple(p2fRectPoints[3]), tuple(p2fRectPoints[0]), Main.SCALAR_GREEN, 2)

            cv2.imshow("4a-este", imgContours)
